...
module guide_fsm #(
  ...
  parameter int BACK_MS   = 2000,  // reverse
  parameter int R1_MS     = 3000,  // first RIGHT arc
  parameter int L1_MS     = 800,   // small LEFT arc
  parameter int COAST_MS  = 1800,  // straight segment
  parameter int L2_MS     = 5000,  // continue LEFT arc
  parameter int R2_MS     = 3400,  // final RIGHT arc to straighten
  ...
  input  logic zebra_pattern_stop,    // raw level from vision
  ...
  // ---- Edge-detect & one-shot re-arm: zebra ----
  logic zebra_stop_d, zebra_rise, zebra_rearm_ok;
  always_ff @(posedge clk or negedge rst_n) begin
    if (!rst_n) zebra_stop_d <= 1'b0;
    else        zebra_stop_d <= zebra_pattern_stop;
  end
  assign zebra_rise = zebra_pattern_stop & ~zebra_stop_d;
  ...
  // ---------- Sequential: state, timers, latches, zebra rearm ----------
  always_ff @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
      s <= S_IDLE;
      started <= 1'b0;
      ...
      // obstacle re-arm: block retrigger during ANY maneuver; require input LOW to rearm
      if (in_maneuver)        obst_rearm_ok <= 1'b0;
      else if (!obstacle_stop)obst_rearm_ok <= 1'b1;

      // zebra re-arm: when in S_ZEBRA, block retrigger; re-arm only after input goes LOW
      if (s == S_ZEBRA)             zebra_rearm_ok <= 1'b0;
      else if (!zebra_pattern_stop) zebra_rearm_ok <= 1'b1;
  ...
  always_comb begin
    ...
    unique case (s)
      ...
      S_FWD: begin
        if (zebra_rise && zebra_rearm_ok)        ns = S_ZEBRA;   // one-shot 3 s stop
        else if (obstacle_rise && obst_rearm_ok) ns = S_BACK;    // begin obstacle maneuver
        else                                     ns = S_FWD;
      end
      ...
      S_ZEBRA:   ns = (zebra_ctr >= Z_T   ) ? S_FWD    : S_ZEBRA; // exactly 3 s, then resume
