...
module fft_pitch_detect_tb;
    ...
    localparam TBCLK    = TXCK * 6; // 3.072 MHz
    ...
    initial begin
        ...
        $dumpvars(0, fft_pitch_detect_tb);
        ...
        repeat (10) @(posedge pitch_output_valid);
        #(TXCK*100);
    ...
    always_ff @(posedge fft_clk) begin
		  if (pitch_output_valid && shown < 20) begin
            freq_hz = pitch_output_data * (12000.0 / NSamples);
            $display("[%0t ps] DETECTED PEAK -> k=%0d (~%0.1f Hz), PEAK_MAG=%0d",
                     $time, pitch_output_data, freq_hz, peak_display);
            shown++;
        end
    end
